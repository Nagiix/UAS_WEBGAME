...
function createHeader() {
  ...
  nav.innerHTML = `
    <div class="container-fluid d-flex justify-content-between align-items-center">
      ...
      <button class="btn btn-secondary" onclick="showScoreboard()">View Scoreboard</button>
...
registerBtn.onclick = () => {
  ...
  loginBtn.onclick = async () => {
    ...
    if (data.status === "success") {
      ...
      if (nickname) {
        ...
        document.getElementById("loginSuccessMessage").innerHTML = `Welcome, <strong>${nickname}</strong>!`;
        ...
        document.getElementById("submitNicknameBtn").onclick = async () => {
          const nickname = document.getElementById("nicknameInput").value.trim();
          if (!nickname) return alert("Please enter a nickname!");

          const res2 = await fetch("api/set_nickname.php", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
          ...
          if (result.status === "success") {
            ...
            alert("❌ Failed to set nickname: " + result.message);
          }
        };
      }
    } else {
  ...
  // ✅ Proper place to handle register form submission
